...
input_data = ['Anystyle', 'Cermine', 'ExCite', 'Grobid', 'Pdfssa4met', 'Scholarcy', 'ScienceParse', 'bibex']
#input_data = ['bibex']  # TEMPORANEO
path_to_gs = 'D:/Projects/Masterarbeit/evaluation/references_extraction_DATA/goldStand_parsed/gold_standard_files/'
path_to_output = 'D:/Projects/Masterarbeit/evaluation/references_extraction_DATA/goldStand_parsed/parsed_output_files/'
...
def compute_values():
    ...
    for parser in final_data:
        ...
        total_comput = [{}]
        for key in keys:
            precision = round(parser[1][key[0]+'_tot_corr'] / parser[1][key[0]+'_tot_out'], 3) if parser[1][key[0]+'_tot_out'] else 0
            recall = round(parser[1][key[0]+'_tot_corr'] / parser[1][key[0]+'_tot_gs'], 3) if parser[1][key[0]+'_tot_gs'] else 0
            f_score = round((2 * (precision * recall) / (precision + recall)), 3) if precision or recall else 0
