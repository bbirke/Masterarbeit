<template>
  <div class="main">
    <v-file-input class="limited-text" accept="application/pdf" label="File input" prepend-icon="mdi-file-pdf-box"
      truncate-length="15" show-size :rules="rules" v-model="files"></v-file-input>
    <v-container>
      <v-row no-gutters>
        <v-col cols="4">
          <v-checkbox v-model="ocr" label="Apply OCR.">
            <v-tooltip activator="parent" location="bottom">Apply OCR on your provided PDF document (Note: this may take a
              little longer).</v-tooltip></v-checkbox>
        </v-col>
        <v-col>
          <v-select v-model="lang_select" label="OCR Language" :items="languages" item-title="lang" item-value="abbr"
            :disabled="!ocr" return-object></v-select>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="4">
        ...
        <v-col>
          <v-text-field v-model="titlepage" single-line type="number" :disabled="!titlepage_offset" />
        </v-col>
      </v-row>
    </v-container>
...
export default {
  ...
  methods: {
    ...
    async uploadFiles() {
      ...
      await axios.post(import.meta.env.VITE_BIBEX_API_ENDPOINT +'/process/pdf/', formData, {
        headers: {
